...
|----------|-------------|---------|----------|------------|
| `PORT` | HTTP server listening port | `8086` | No | Integer 1-65535 |
| `ENV` | Deployment environment | `development` | Yes | One of: development, staging, production |
| `SERVICE_NAME` | Service identifier for logs/metrics | `search-service` | No | Non-empty string |
...
| `KAFKA_CONSUMER_GROUP` | Consumer group ID | `search-service` | No | Non-empty string |
| `KAFKA_TOPICS` | Topics to consume | `auction.events,bid.events` | No | CSV of topic names |
| `KAFKA_DLQ_TOPIC` | Dead Letter Queue topic | `search.events.dlq` | No | Non-empty string |
...
| `VAULT_ROLE` | Vault Kubernetes auth role | `search-service` | No | Non-empty string |
| `VAULT_SECRET_PATH` | Path to service secrets in Vault | `secret/data/auction/search-service` | No | Valid Vault path |
